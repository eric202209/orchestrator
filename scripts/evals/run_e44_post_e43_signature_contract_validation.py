#!/usr/bin/env python3
"""E44: Post-E43 Bounded Debug Repair Signature Contract Validation.

Runs 10 sequential medium_cli_multi_file_feature tasks with E43 code loaded
and measures whether the strengthened signature contract prevents wrong-signature
repairs.

E41 baseline (medium_cli, 10 tasks, E40 code):
- completed: 0/10
- PAEF: 8/10
- BACKEND_CAPACITY: 0/10
- E40-path correct format_summary sig: 1/3
- all-task correct format_summary sig: 8/10 (inflated — non-repair paths)
- TaskStore.summary IMPL: 1/10
- format_task_line preserved: 10/10

E43 change: replaced weak escape-hatch header with authoritative-signature rule;
added Rule 12 (src-contract) and Rule 11/12 (no-src-contract) forbidding
parameter-list changes on stub functions.

Decision threshold:
- E40-path correct sig >= 80% → KEEP_E43
- E40-path correct sig improved but < 80% → KEEP_E43_WITH_MINOR_FIX or MONITOR_MORE
- E40-path correct sig ~1/3 despite prompt text present → PROCEED_TO_POST_REPAIR_SIGNATURE_GUARD
- Regressions (format_task_line < 9/10) → REVERT_E43
"""
...
OUTPUT_FILE = Path("/tmp/e44-results.json")
...
def _fresh_workspace(fixture: str, tag: str) -> Path:
    ...
    dest = WORKSPACE_ROOT / f"e44-{fixture.replace('_', '-')}-{tag}"
    if dest.exists():
        shutil.rmtree(dest)
...
def _orchestration_event_signals(workspace_path: Path, session_id: int, task_id: int) -> dict:
    """Read JSONL orchestration event log for bounded-debug-repair signals.

    Note: JSONL event types are stored lowercase (e.g. "debug_repair_attempted"),
    not uppercase. This was confirmed in E41 (the E41 eval script had a case-sensitivity
    bug; this script uses lowercase to match the stored values).
    """
    ...
    for line in lines:
        ...
        # Event types are stored lowercase in JSONL (E41 confirmed case-sensitivity bug)
        event_type = str(event.get("event_type", "")).lower()
        details = event.get("details", {})
        debug_scope = str(details.get("debug_repair_scope", "")).lower()

        # Detect bounded execution debug repair (lowercase event type)
        if event_type == "debug_repair_attempted" and (
            ...
            if result["fallback_reason"] is None:
                result["fallback_reason"] = details.get("fallback_reason")
        ...
        # Detect repair_generated with hash observability (lowercase)
        ...
        # Detect repair_rejected (lowercase)
        if event_type == "repair_rejected":
            reason = details.get("rejection_reason") or details.get("debug_repair_terminal_reason")
            if reason:
                result["repair_rejected_reasons"].append(str(reason))
            if (
...
def _inspect_workspace(workspace_path: Path) -> dict:
    ...
    result = {
        ...
        "format_summary_signature_found": None,
    ...
    if store_py.exists():
        try:
            ...
            if "def summary(" in store_text:
                if "raise NotImplementedError" not in store_text:
                ...
                try:
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, ast.FunctionDef) and node.name == "format_summary":
                            ...
                            result["format_summary_signature_found"] = str(arg_names)
...
def _e43_in_process_check() -> bool:
    """Verify E43 signature contract text is loaded in this Python process."""
    ...
    checks = {
        "E40 changed_file_context_present field": "bounded_execution_debug_repair_changed_file_context_present" in src_helper,
        "E40 CURRENT CONTENT section": "CURRENT CONTENT OF IMPLICATED SOURCE FILES" in src_helper,
        "E43 authoritative-signature header": "Existing function signatures shown here are authoritative" in src_helper,
        "E43 no escape hatch": "unless the failure requires a change" not in src_helper,
        "E43 signature rule in main fn": "implement its body only" in src_main,
        "E43 parameter list rule": "Do not change its parameter list" in src_main,
        "E40 main fn calls helper": "build_bounded_debug_repair_changed_file_context" in src_main,
        "E40 metadata merged": "changed_file_context_metadata" in src_main,
    }
    all_pass = True
...
def main() -> None:
    ...
    # ── E43 in-process verification ──────────────────────────────────────────
    print("[E44] E43 in-process code verification:")
    if not _e43_in_process_check():
        print("  ERROR: E43 not loaded. Worker restart required before eval.")
        sys.exit(1)
    ...
    for i, task_spec in enumerate(TASK_CORPUS):
        ...
        tid = task_spec["id"]
        print(f"\n[E44] ({i+1}/{len(TASK_CORPUS)}) {tid} fixture={fixture}", flush=True)
        ...
        try:
            proj = _api("POST", "projects", token, {
                "name": f"E44 {tid}",
                "description": f"E44 post-E43 signature contract validation {tid}",
            ...
            session = _api("POST", "sessions", token, {
                "project_id": proj_id,
                "name": f"e44-{task_spec['id'].lower()}-{tag}",
        ...
        fallback_reason = event_signals.get("fallback_reason")
        repair_rejected_reasons = event_signals.get("repair_rejected_reasons", [])
        ...
        rec = {
            ...
            "format_summary_signature_found": ws_signals.get("format_summary_signature_found"),
        ...
        print(
            ...
            f"changed_paths={repair_output_changed_paths} "
            f"store.summary={'IMPL' if ws_signals['store_summary_implemented'] else 'STUB'} "
            f"fmt_sig={'OK' if ws_signals['format_summary_correct_signature'] else 'BAD'}({ws_signals.get('format_summary_signature_found')}) "
            f"ftl={'OK' if ws_signals['format_task_line_preserved'] else 'MISSING'} "
            f"pytest={'PASS' if ws_signals['pytest_passed'] else 'FAIL'}",
            flush=True,
        )

    OUTPUT_FILE.write_text(json.dumps(results, indent=2), encoding="utf-8")
    print(f"\n[E44] Raw results → {OUTPUT_FILE}")
    ...
    # E40-path sub-population: tasks where changed_file_context_present=True
    e40_path_pop = [r for r in valid if r.get("changed_file_context_present")]
    e40_path_size = len(e40_path_pop)
    fmt_sig_in_e40_path = sum(1 for r in e40_path_pop if r.get("format_summary_correct_signature"))
    store_in_e40_path = sum(1 for r in e40_path_pop if r.get("store_summary_implemented"))

    # All bounded-repair sub-population
    ...
    print("\n" + "=" * 70)
    print("[E44] AGGREGATES (vs E41 baseline)")
    print("=" * 70)
    print(f"  Total tasks:                              {total}/10")
    print(f"  completed:                                {completed_count}/10  (E41: 0/10)")
    print(f"  PLAN_ACCEPTED_EXECUTION_FAILED (PAEF):    {paef_count}/10  (E41: 8/10)")
    print(f"  BACKEND_CAPACITY:                         {backend_count}/10  (E41: 0/10)")
    print(f"  planning_circuit_breaker:                 {oscillation_count}/10  (E41: 0/10)")
    print(f"  planning_json_parse_failure:              {json_fail_count}/10  (E41: 0/10)")
    print(f"  bootstrap_contract_PRCF:                  {prcf_count}/10  (E41: 0/10)")
    print(f"  other:                                    {other_count}/10")
    print()
    print(f"  execution_reached:                        {exec_reached_count}/10  (E41: 10/10)")
    print(f"  bounded_execution_debug_repair_invoked:   {bounded_repair_count}/10  (E41: 9/10)")
    print()
    print("[E44] E40-PATH SIGNALS (changed_file_context_present=True)")
    print(f"  E40-path tasks (ctx_present=True):        {e40_path_size}/10  (E41: 3/10)")
    if e40_path_size > 0:
        rate = fmt_sig_in_e40_path / e40_path_size
        print(f"  E40-path correct format_summary sig:      {fmt_sig_in_e40_path}/{e40_path_size} = {rate:.0%}  (E41: 1/3 = 33%)")
        print(f"  E40-path store.summary implemented:       {store_in_e40_path}/{e40_path_size}  (E41: 1/3)")
    print()
    print("[E44] WORKSPACE QUALITY (all tasks)")
    print(f"  store.summary() implemented:              {store_impl_count}/10  (E41: 1/10)")
    print(f"  format_summary(total,completed) OK:       {fmt_sig_ok_count}/10  (E41: 8/10)")
    print(f"  format_task_line preserved:               {fmt_task_preserved_count}/10  (E41: 10/10)")
    print(f"  pytest passed in workspace:               {pytest_pass_count}/10  (E41: 0/10)")
    print(f"  changed_file_context_present (any):       {ctx_present_count}/10  (E41: 3/10)")
    print(f"  repair_output_hash_present:               {repair_hash_count}/10  (E41: 2/10)")
    print()
    if repair_pop_size > 0:
        print("[E44] WORKSPACE QUALITY (all bounded debug repair sub-population)")
        print(f"  format_summary sig OK:                    {fmt_in_repair_pop}/{repair_pop_size}  (E41: 1/2 hash-present)")
        print(f"  store.summary implemented:                {store_in_repair_pop}/{repair_pop_size}")
        print(f"  ctx_present in repair pop:                {ctx_in_repair_pop}/{repair_pop_size}")
        print()
    print(f"  budget_exceeded (>6000):                  {budget_fail_count}/10  (target: 0)")
    print(f"  max_repair_prompt_chars:                  {max_chars}  (limit: 6000)")

    print("\n" + "=" * 70)
    print("[E44] PER-TASK RESULTS")
    print("=" * 70)
    hdr = (
        f"{'ID':<12} {'task_id':<8} {'sess':<6} {'status':<10} {'category':<35} "
    ...
    for r in valid:
        ...
        if r.get("changed_file_context_present"):
            print(
                f"    [E40-path] paths={r['changed_file_context_paths']} "
                f"chars={r['changed_file_context_chars']} "
                f"fallback={r.get('fallback_reason')} "
                f"fmt_sig_found={r.get('format_summary_signature_found')}"
            )

    # ── Decision rule ─────────────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("[E44] DECISION RULE EVALUATION")
    print("=" * 70)

    api_regressions = fmt_task_preserved_count < 9  # E41 was 10/10; allow 1 margin

    if e40_path_size > 0:
        e40_path_sig_rate = fmt_sig_in_e40_path / e40_path_size
    else:
        e40_path_sig_rate = None

    sig_improved_vs_e41 = (
        e40_path_sig_rate is not None and e40_path_sig_rate > (1 / 3)
    )
    sig_threshold_met = (
        e40_path_sig_rate is not None and e40_path_sig_rate >= 0.80
    )
    sig_stagnant = (
        e40_path_size > 0 and not sig_improved_vs_e41
    )

    print(f"  E40-path tasks observed:                    {e40_path_size}")
    if e40_path_size > 0:
        print(f"  E40-path correct sig rate:                  {e40_path_sig_rate:.0%}  (E41: 33%)")
        print(f"  >= 80% threshold (KEEP_E43):                {'YES' if sig_threshold_met else 'NO'}")
        print(f"  Improved vs E41 baseline (33%):             {'YES' if sig_improved_vs_e41 else 'NO'}")
    else:
        print("  WARNING: No E40-path cases observed — cannot score primary metric")
    print(f"  format_task_line regressions (< 9/10):      {'YES' if api_regressions else 'NO'} ({fmt_task_preserved_count}/10)")

    print()
    if api_regressions:
        recommendation = "REVERT_E43"
        rationale = f"format_task_line regressions detected ({fmt_task_preserved_count}/10) — API weakening."
    elif e40_path_size == 0:
        recommendation = "MONITOR_MORE"
        rationale = "No E40-path cases in this batch — cannot evaluate signature contract effect. Run another batch."
    elif sig_threshold_met:
        recommendation = "KEEP_E43"
        rationale = f"E40-path correct sig rate {e40_path_sig_rate:.0%} >= 80% threshold."
    elif sig_improved_vs_e41 and not sig_threshold_met:
        recommendation = "KEEP_E43_WITH_MINOR_FIX"
        rationale = (
            f"E40-path sig improved ({e40_path_sig_rate:.0%}) vs E41 (33%) but below 80% threshold. "
            "Consider additional prompt strengthening before escalating to Option B."
        )
    elif sig_stagnant:
        recommendation = "PROCEED_TO_POST_REPAIR_SIGNATURE_GUARD"
        rationale = (
            f"E40-path sig rate ({e40_path_sig_rate:.0%}) not improved vs E41 (33%) despite E43 prompt text present. "
            "Model cannot be instructed to preserve signatures; implement hard rejection guard (Option B)."
        )
    else:
        recommendation = "MONITOR_MORE"
        rationale = "Insufficient E40-path cases for confident determination."

    print(f"  Recommendation: {recommendation}")
    print(f"  Rationale: {rationale}")
    print()
    print(f"[E44] Raw results saved to {OUTPUT_FILE}")
