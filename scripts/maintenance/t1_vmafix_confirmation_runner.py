#!/usr/bin/env python3
"""
verification_mutates_source_assets Fix — Confirmation Run (+ Path Guard Run 3/3).

Purpose: confirm the vma repair-deadlock fix (repair_prompts.py) and the
verification-profile planning contract (planner.py) eliminate the Class B
T6 "Final verification" failure, per
docs/roadmap/reports/maintenance/verification-mutates-source-assets-fix-20260611.md.

Same 6-task T1-corpus as prior runs.
Fresh workspaces: t1-vmafix-{calclib,pathtools,strtools}.
...
Primary metrics:
- >= 2/3 T6 tasks DONE, or vma occurrence with repair that removes write ops
- verification_mutates_source_assets count = 0 expected
- nested_project_folder_command = 0, advisory = 0, pip-show recurrence = 0
"""
import copy
import json
import pathlib
...
def vmafix_projects() -> list:
    projects = copy.deepcopy(r.PROJECTS)
    for p in projects:
        p["name"] = p["name"].replace("t1-confirm-", "t1-vmafix-")
        p["workspace"] = p["workspace"].replace("t1-confirm-", "t1-vmafix-")
        p["description"] = p["description"].replace(
            "T1 reliability confirmation — venv pip show fix verification",
            "vma repair-deadlock fix + verification-profile planning contract confirmation",
        )
    return projects
...
if __name__ == "__main__":
    ...
    for proj_spec in vmafix_projects():
        print(f"\n{'='*60}")
    ...
    out_path = out_dir / f"t1-vmafix-raw-{run_ts}.json"
    out_path.write_text(json.dumps({"meta": run_meta, "results": all_results}, indent=2))
    print(f"\n\nRaw results saved: {out_path}")

    print("\n" + "=" * 60)
    print("VMA FIX CONFIRMATION + PATH GUARD RUN 3 SUMMARY")
    print("=" * 60)
