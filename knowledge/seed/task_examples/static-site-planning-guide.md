...
- For verification-only or review tasks, never invent conventional asset names such as `style.css`, `app.css`, `garden.svg`, or `logo.svg`. Use the paths that are actually present in the current workspace, including nested paths such as `css/style.css` or `images/flower-bg.svg`.
