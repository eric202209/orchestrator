...
def _runtime_failure_classification(exc: Exception) -> str:
    """Return the failure-classifier result needed by runtime evidence.

    The former orchestration classifier received no orchestration state here,
    so only its exception/diagnostic rules can affect this result.  Keeping
    that bounded projection local avoids importing orchestration lifecycle
    policy into the execution authority.
    """

    provider_classification = getattr(exc, "provider_failure_classification", None)
    if provider_classification:
        return str(provider_classification)
    diagnostics = getattr(exc, "runtime_diagnostics", None) or {}
    exc_text = str(exc)
    lower_text = exc_text.lower()
    if any(cls.__name__ == "ProjectMutationLockError" for cls in type(exc).__mro__):
        return "project_mutation_lock_conflict"
    debug_mode = diagnostics.get("debug_prompt_mode_architecture")
    if debug_mode is None:
        debug_mode = diagnostics.get("debug_prompt_mode")
    if (
        debug_mode in {"bounded_execution_debug_repair", "phase7f_bounded_debug_repair"}
        and str(diagnostics.get("failure_phase", "")).lower()
        in {"debug_repair", "orchestrationphase.debug_repair"}
        and diagnostics.get("debug_failure_class") == "source_step_validation"
        and (
            diagnostics.get("timed_out") is True
            or "timed out" in lower_text
            or "timeout" in lower_text
        )
    ):
        return "bounded_debug_repair_timeout"
    if (
        diagnostics.get("timeout_boundary") == "planning_lock_wait"
        or "openclaw planning lock" in lower_text
    ):
        return "planning_lock_contention"
    if any(marker in lower_text for marker in ("time limit", "timeout", "timed out")):
        return "execution_timeout"
    if "parse" in lower_text:
        return "debug_parse_error"
...
class ExecutionTaskRuntimeExecutionService:
    ...
    def verify_runtime_start_integrity(
        ...
        return verify_runtime_start_integrity(self.db, runtime_start_id)

    def verify_attempt_outcome_integrity(
        self, outcome_id: int
    ) -> RuntimeIntegrityResult:
        return verify_attempt_outcome_integrity(self.db, outcome_id)
    ...
    def _failure_result(exc: Exception) -> RuntimeExecutionResult:
        raw_category = _runtime_failure_classification(exc)
