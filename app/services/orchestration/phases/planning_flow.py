...
from app.services.orchestration.phases.planning_operation_repair_control import (
    attempt_operation_level_repair,
)
    ...
    try:
        while True:
            ...
            if blocking_repair_issues and not retry_state.repair_prompt_used:
                operation_repair_control = attempt_operation_level_repair(
                    ctx=ctx,
                    retry_state=retry_state,
                ...
                if operation_repair_control.get("action") == "return":
                    return operation_repair_control["result"]
                if operation_repair_control.get("action") == "continue":
                    planning_result = operation_repair_control["planning_result"]
                    continue
                contract_violations = (
                    PlannerService.describe_planning_contract_violations(
                        output_text=output_text,
