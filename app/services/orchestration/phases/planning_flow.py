    ...
    *,
    ...
    try:
        while True:
            ...
            if retry_state.repair_prompt_used and not getattr(
                retry_state, "operation_repair_succeeded", False
            ):
            ...
            if blocking_repair_issues and not retry_state.repair_prompt_used:
                ...
                operation_findings = list(
                    (operation_plan_verdict.details or {}).get(
                ...
                if operation_prompt is not None:
                    emit_phase_event(
                        ...
                        details={
                            ...
                            "rejected_operation_identities": list(
                                operation_route.rejected_identities
                            ),
                            "repair_prompt_chars": len(operation_prompt),
                            "repair_prompt_estimated_tokens": (
                                len(operation_prompt) + 3
                            )
                            // 4,
                    ...
                    try:
                        ...
                        operation_merge_result = merge_and_validate_operation_repairs(
                            ...
                            rejected_findings=operation_findings,
                            response_text=str(
                                operation_provider_result.get("output") or ""
                            ),
                            source_materialization=(ctx.planner_source_materialization),
                            project_dir=ctx.orchestration_state.project_dir,
                            validate_complete_plan=lambda plan: (
                                _validate_planning_plan(ctx, plan, json.dumps(plan))
                            ),
                        )
                    except Exception as exc:
                        ctx.orchestration_state.status = OrchestrationStatus.ABORTED
                        ...
                        _finalize_planning_terminal_failure(
                            ...
                            failure_reason=(
                                "Operation-level Planning repair failed closed "
                                f"after at most one provider request: {type(exc).__name__}: "
                                f"{str(exc)[:500]}"
                            ),
                    ...
                    output_text = json.dumps(operation_merge_result.plan)
                    retry_state.repair_prompt_used = True
                    ...
                    emit_phase_event(
                        ...
                        details={
                            ...
                            "validator_status": getattr(
                                operation_merge_result.validator_verdict,
                                "status",
                                None,
                            ),
                        },
                    )
                    # The operation lane has consumed the only repair request and
                    # already revalidated the complete merged Plan. Continue this
                    # loop with the merged Plan as the parser input, without
                    # entering complete-plan repair.
                    planning_result = {
                        "status": "completed",
                        "output": output_text,
                    }
            ...
            _p2b = _run_guidance_plan_enforcement(
                ctx,
                retry_state=retry_state,
                output_text=output_text,
                planning_timeout_seconds=planning_timeout_seconds,
                prompt_profile=prompt_profile,
                repair_fn=__repair_planning_output,
